...
.book { width:110px; min-height:160px; background:#111; padding:6px; border-radius:8px; border:1px solid #555; text-align:center; word-wrap:break-word; flex:0 1 auto; }
...
function filterBooks(){
  ...
  document.querySelectorAll('#grid .book').forEach(book=>{
    ...
    book.style.display = (title.includes(query)||author.includes(query))?'inline-block':'none';
  });
}
</script>
...
app.get('/', async(req,res)=>{
  try{
    ...
    const libros = files.map(f=>bookMetadata.find(b=>b.id===f.id)).filter(Boolean);
    res.send(renderBookPage({ libros, titlePage:'🪄 Azkaban Reads', tipo:'libros', nombre:'', req }));
  }catch(err){
    console.error(err);
...
app.get('/autores',(req,res)=>{
  ...
  const authorsHtml = autores.map(a=>`<div class="book"><div class="title">${a}</div><div class="meta"><a href="/autor?name=${encodeURIComponent(a)}">Ver libros</a></div></div>`).join('');
  res.send(`<html><head><style>${css}</style></head><body>
<div class="header-banner"></div>
<h1>Autores</h1>
<div class="top-buttons"><a href="/" class="button">🪄 Libros</a><a href="/sagas" class="button">Sagas</a></div>
...
app.get('/sagas',(req,res)=>{
  ...
  const sagasHtml = sagas.map(s=>`<div class="book"><div class="title">${s}</div><div class="meta"><a href="/saga?name=${encodeURIComponent(s)}">Ver libros</a></div></div>`).join('');
  res.send(`<html><head><style>${css}</style></head><body>
<div class="header-banner"></div>
<h1>Sagas</h1>
<div class="top-buttons"><a href="/" class="button">🪄 Libros</a><a href="/autores" class="button">Autores</a></div>
